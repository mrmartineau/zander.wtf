...
### Desktop shell, terminal mode and classic mode

Three front ends share the same pages. `BaseLayout.astro` renders the classic header, the greeting (`Greeting.astro`), `<main id="page">` and the footer. The URL prefix picks the front end — `/cv` is the website, `/desktop/cv` the desktop, `/tui/cv` the terminal, `/txt/cv` plain text with every stylesheet stripped by the inline script (`src/utils/uiMode.ts`). `public/_redirects` rewrites `/desktop/*`, `/tui/*` and `/txt/*` to the plain pages on Cloudflare (200 proxying); a tiny integration in `astro.config.mjs` does the same in dev. An inline script reads the prefix and sets `html.desktop`, `html.tui` or `html.classic`; the shells strip the prefix from every URL they handle and add it back in `history`; in desktop and terminal mode the header/footer are hidden. **Terminal mode** is a second Solid island (`src/components/solid/tui/Tui.tsx`, styles in `tui.css`): a raspi-config style menu over the same pages, driven by ↑/↓, Tab, ←/→, Enter and Esc, with links inside a page as the rows; it reuses `page.ts` from the desktop. In desktop mode a Solid island (`src/components/solid/desktop/`, `client:only`) mounts a classic-Mac style desktop over it and adopts that `<main>` as the first window (the greeting goes behind the windows). The home page is made of `[data-window]` sections, which open as one window each (`split()` in `page.ts`); `/#about` opens just that section. The Z menu, the terminal's Display screen and the `ModeCard` on the classic home page carry the switches between modes. Pages stay ordinary Astro pages: links inside a window are intercepted, the target page is fetched, its `<main>`, stylesheets and hoisted scripts are pulled out of the response (`page.ts`) and shown in a new window. `store.ts` owns window state and keeps `history`/`document.title` pointing at the front window, so every URL still deep-links and works without JS (a `<noscript>` nav replaces the header). Spotlight (`⌘K`) uses `/api/search`. Sounds are synthesised with Web Audio (`sound.ts`), off by default. Styles live in `src/styles/components/desktop.css`; ZUI's semantic tokens are pointed at the site palette there and `--radius-scale` is zeroed for hard corners.
