...
subtitle: Ten side projects, npm packages, Raycast extensions, iOS apps and more, plus what using Claude Code, Cursor and Amp on real work taught me
date: 2026-02-23
tags:
  - side-project
  - ai
...
The fix was to build proper native extensions: a Safari extension and a Share Extension. Both give Otter a first-class place in iOS's sharing flow, and the experience is far better. Tap, share, done. No more permission dialogs.
...
My code notes, a collection of TILs and snippets I've accumulated over the years, used to live on a separate Eleventy site. It worked, but maintaining two separate sites with different toolchains and deployment pipelines was more friction than it needed to be. I migrated the whole thing into this Astro site, which means one repo, one build, one deployment. Far less to maintain.

The one wrinkle was search. It's powered by a Cloudflare Worker and worked perfectly in local development, but getting it to behave correctly once hosted took some head-scratching. Production has a way of finding edge cases that local dev cheerfully ignores.
...
What I like most is that it pulls the same data as the main site rather than being a static snapshot, so it stays up to date automatically. New blog posts, new code notes, whatever I add to the site proper shows up in the ASCII version too. It's a small detail but it makes it feel like a real alternative view of the site rather than a gimmick.

## On the AI agents themselves

All three tools got a proper workout across these projects, and I've come away with some firm opinions.

**[Amp](https://ampcode.com) is my favourite.** It has access to both Anthropic and OpenAI models, so it can pick whichever suits the task. In practice that means better results across a wider range of problems, without me having to think about which model to reach for. I trust it with bigger chunks of work than the other two.

**Cursor's "Add to chat" is brilliant for refactoring.** Being able to select a specific block of code and ask a targeted question about it, without pulling in the entire file, makes Cursor really good at surgical edits. It's the feature I reach for most often when I'm using it. I don't use Cursor's CLI agent as much, partly because Amp fills that role for me, but I should probably give it more of a chance.

**Claude Code's** TUI is really problematic compared to Amp, so I rarely use it. I do like its iOS app though: I can raise PRs on existing projects while I'm out. I used that for Otter and ascii-img-react, with good results.
