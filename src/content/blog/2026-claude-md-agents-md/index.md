...
- The research is thinner and less flattering than the blog posts suggest. Developer-written context files help a little. LLM-generated ones (hello, `/init`) measurably hurt. Most of what `/init` writes should go: the overview, the architecture summary, the file map. The commands and the genuine gotchas should stay.
...
Underneath the noise there's a durable core. Roughly everyone agrees on these: the official docs, the minimalists, even most of the template-shaped posts.
...
HumanLayer's argument is that `CLAUDE.md` is the highest-leverage point of the whole harness, so it should be hand-crafted. That was a taste argument in November 2025. As of early 2026 there's data behind it, which is the next section. There's now a much more direct answer to "what should I delete" too, which gets its own section below.
...
Worth noticing what Anthropic did about that in their own harness, though: **Explore and Plan skip `CLAUDE.md` entirely**, and the docs say it's to keep research "fast and inexpensive". When the built-in agents whose job is reading code don't get your context file at all, that tells you how much of it Anthropic thinks is load-bearing for a task that isn't writing code.

The one place you *can* vary things is the subagent definition. A file in `.claude/agents/` takes a `model` field (`sonnet`, `opus`, `haiku`, `fable`, a full model ID, or `inherit`) alongside its own system prompt in the markdown body, its own `effort` level, and a `skills` list that preloads full skill content at startup. So the extra hand-holding a cheaper model needs goes *in the agent file that pins that model*, right next to the line pinning it:
...
There's a cost wrinkle here that's easy to miss. If you're routing work to Haiku to save money, remember your `CLAUDE.md` is loaded **per subagent, per spawn**. Fan out ten subagents and you've paid for that file eleven times. Bloat that's merely annoying in a single-threaded session becomes a multiplier the moment you parallelise. A heavy fan-out workflow is one of the stronger arguments for a short file, not a weaker one.
...
| E | Everything in `CLAUDE.md` vs skills | Pre-2026 posts, because skills didn't exist | Route conditional context to skills and path-scoped rules | Not a real disagreement, just a timeline artefact |
